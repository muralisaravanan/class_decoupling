...
int input_read_parameters(
  ...
  /** - dark decoupled non-cold relics (ddec) */
  class_read_int("N_ddec",N_ddec);
  if ((flag1 == _TRUE_) && (N_ddec > 0)){
    pba->N_ddec = N_ddec;

    if (ppt->gauge == synchronous)
      ppr->tol_ddec = ppr->tol_ddec_synchronous;
    if (ppt->gauge == newtonian)
      ppr->tol_ddec = ppr->tol_ddec_newtonian;

    /* Quadrature modes, 0 is qm_auto. */
    class_read_list_of_integers_or_default("Quadrature strategy",pba->ddec_quadrature_strategy,0,N_ddec);
    /* Number of momentum bins */
    class_read_list_of_integers_or_default("Number of momentum bins",pba->ddec_input_q_size,-1,N_ddec);

    /* qmax, if relevant */
    class_read_list_of_doubles_or_default("Maximum q",pba->ddec_qmax,15,N_ddec);

    /* Read temperatures: */
    class_read_list_of_doubles_or_default("T_ddec",pba->T_ddec,pba->T_ddec_default,N_ddec);

    /* Read dark decoupling (ddc) parameters: */
    class_read_list_of_doubles_or_default("zdec_ddec",pba->zdec_ddec,pba->zdec_ddec_default,N_ddec);
    class_read_list_of_doubles_or_default("dzdec_ddec",pba->dzdec_ddec,pba->dzdec_ddec_default,N_ddec);
    class_read_list_of_integers_or_default("ddec_fermion_boson",pba->ddec_fermion_boson,pba->ddec_fermion_boson_default,N_ddec);

    /* Read chemical potentials: */
    class_read_list_of_doubles_or_default("ksi_ddec",pba->ksi_ddec,pba->ksi_ddec_default,N_ddec);

    /* Read degeneracy of each ddec species: */
    class_read_list_of_doubles_or_default("deg_ddec",pba->deg_ddec,pba->deg_ddec_default,N_ddec);

    /* Read mass of each ddec species: */
    class_read_list_of_doubles_or_default("m_ddec",pba->m_ddec_in_eV,0.0,N_ddec);

    /* Read Omega of each ddec species: */
    class_read_list_of_doubles_or_default("Omega_ddec",pba->Omega0_ddec,0.0,N_ddec);

    /* Read omega of each ddec species: (Use pba->M_ddec temporarily)*/
    class_read_list_of_doubles_or_default("omega_ddec",pba->M_ddec,0.0,N_ddec);

    /* Check for duplicate Omega/omega entries, missing mass definition and
       update pba->Omega0_ddec:*/
    for(n=0; n<N_ddec; n++){
      /* pba->M_ddec holds value of omega */
      if (pba->M_ddec[n]!=0.0){
        class_test(pba->Omega0_ddec[n]!=0,errmsg,
                   "Nonzero values for both Omega and omega for ddec species %d are specified!",n);
        pba->Omega0_ddec[n] = pba->M_ddec[n]/pba->h/pba->h;
      }
      if ((pba->Omega0_ddec[n]==0.0) && (pba->m_ddec_in_eV[n]==0.0)) {
        /* this is the right place for passing the default value of
           the mass (all parameters must have a default value; most of
           them are defined in input_default_params{}, but the ddec mass
           is a bit special and there is no better place for setting its
           default value). We put an arbitrary value m << 10^-3 eV,
           i.e. the ultra-relativistic limit.*/
        pba->m_ddec_in_eV[n]=1.e-5;
      }
    }

    /* Check if filenames for interpolation tables are given: */
    class_read_list_of_integers_or_default("use_ddec_psd_files",pba->got_files,_FALSE_,N_ddec);

    if (flag1==_TRUE_){
      for(n=0,fileentries=0; n<N_ddec; n++){
        if (pba->got_files[n] == _TRUE_) fileentries++;
      }
      ...
      if (fileentries > 0) {
        ...
        /* Okay, read filenames.. */
        class_call(parser_read_list_of_strings(pfc,"ddec_psd_filenames",
                                               &entries_read,&(pba->ddec_psd_files),&flag2,errmsg),
                   errmsg,
                   errmsg);
        class_test(flag2 == _FALSE_,errmsg,
                   "Input use_ddec_files is found, but no filenames found!");
        class_test(entries_read != fileentries,errmsg,
                   "Number of filenames found, %d, does not match number of _TRUE_ values in use_ddec_files, %d",
                   entries_read,fileentries);
      }
    }
    /* Read (optional) p.s.d.-parameters:*/
    parser_read_list_of_doubles(pfc,
                                "ddec_psd_parameters",
                                &entries_read,
                                &(pba->ddec_psd_parameters),
                                &flag2,
                                errmsg);

    class_call(background_ddec_init(ppr,pba),
               pba->error_message,
               errmsg);
    ...
    /* We must calculate M from omega or vice versa if one of them is missing.
       ...
       reflect the implicit normalization of the distribution function.*/
    for (n=0; n < N_ddec; n++){
      if (pba->m_ddec_in_eV[n] != 0.0){
        /* Case of only mass or mass and Omega/omega: */
        pba->M_ddec[n] = pba->m_ddec_in_eV[n]/_k_B_*_eV_/pba->T_ddec[n]/pba->T_cmb;
        class_call(background_ddec_momenta(pba->q_ddec_bg[n],
                                           pba->w_ddec_bg[n],
                                           pba->q_size_ddec_bg[n],
                                           pba->M_ddec[n],
                                           pba->factor_ddec[n],
                                           0.,
                                           NULL,
                   ...
                   pba->error_message,
                   errmsg);
        if (pba->Omega0_ddec[n] == 0.0){
          pba->Omega0_ddec[n] = rho_ddec/pba->H0/pba->H0;
        }
        else{
          fnu_factor = (pba->H0*pba->H0*pba->Omega0_ddec[n]/rho_ddec);
          pba->factor_ddec[n] *= fnu_factor;
          /* dlnf0dlnq is already computed, but it is
             independent of any normalization of f0.
             We don't need the factor anymore, but we
             store it nevertheless:*/
          pba->deg_ddec[n] *=fnu_factor;
        }
      }
      else{
        /* Case of only Omega/omega: */
        class_call(background_ddec_M_from_Omega(ppr,pba,n),
                   pba->error_message,
                   errmsg);
        //printf("M_ddec:%g\n",pba->M_ddec[n]);
        pba->m_ddec_in_eV[n] = _k_B_/_eV_*pba->T_ddec[n]*pba->M_ddec[n]*pba->T_cmb;
      }
      pba->Omega0_ddec_tot += pba->Omega0_ddec[n];
      //printf("Adding %g to total Omega..\n",pba->Omega0_ddec[n]);
    }
  }
  Omega_tot += pba->Omega0_ddec_tot;
  ...
  */
  ...
  /**if (ppr->ddec_fluid_approximation == ddecfa_fltofs1) {
    ppr->ddec_fluid_trigger_tau_over_tau_k = 0.15;
  }
  else if (ppr->ddec_fluid_approximation == ddecfa_fltofs2) {
    ppr->ddec_fluid_trigger_tau_over_tau_k = 1.e8;
  }**/
  // ddec specific end
  ...
  if (pba->N_ddec>0) {
    ...
    class_test(ppr->ddec_fluid_trigger_tau_over_tau_k==ppr->radiation_streaming_trigger_tau_over_tau_k,
               errmsg,
               "please choose different values for precision parameters ddec_fluid_trigger_tau_over_tau_k and radiation_streaming_trigger_tau_over_tau_k, in order to avoid switching two approximation schemes at the same time");

    class_test(ppr->ddec_fluid_trigger_tau_over_tau_k==ppr->ur_fluid_trigger_tau_over_tau_k,
               errmsg,
               "please choose different values for precision parameters ddec_fluid_trigger_tau_over_tau_k and ur_fluid_trigger_tau_over_tau_k, in order to avoid switching two approximation schemes at the same time");

    class_test(ppr->ddec_fluid_trigger_tau_over_tau_k==ppr->ncdm_fluid_trigger_tau_over_tau_k,
               errmsg,
               "please choose different values for precision parameters ddec_fluid_trigger_tau_over_tau_k and ncdm_fluid_trigger_tau_over_tau_k, in order to avoid switching two approximation schemes at the same time");
...
int input_default_params(
  ...
  pba->N_ddec = 0;
  pba->Omega0_ddec_tot = 0.;
  pba->ksi_ddec_default = 0.;
  pba->ksi_ddec = NULL;
  pba->T_ddec_default = 0.71611; /* this value gives m/omega = 93.14 eV b*/
  pba->T_ddec = NULL;
  // ddec specific start
  pba->zdec_ddec_default = 1e12;
  pba->zdec_ddec = NULL;
  pba->dzdec_ddec_default = 10.;
  pba->dzdec_ddec = NULL;
  pba->ddec_fermion_boson_default = 0;
  pba->ddec_fermion_boson = NULL;
  // ddec specific end
  pba->deg_ddec_default = 1.;
  pba->deg_ddec = NULL;
  pba->ddec_psd_parameters = NULL;
  pba->ddec_psd_files = NULL;
  ...
  pba->K = 0.;
  pba->sgnK = 0;
  pba->Omega0_lambda = 1.-pba->Omega0_k-pba->Omega0_g-pba->Omega0_ur-pba->Omega0_b-pba->Omega0_cdm-pba->Omega0_ncdm_tot-pba->Omega0_dcdmdr-pba->Omega0_ddec_tot;
  pba->Omega0_fld = 0.;
  pba->a_today = 1.;
  pba->use_ppf = _TRUE_;
