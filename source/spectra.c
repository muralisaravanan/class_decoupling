...
int spectra_pk_at_z(
  ...
  if (psp->ln_tau_size == 1) {
    ...
    for (index_k=0; index_k<psp->ln_k_size; index_k++)
      if (psp->ic_size[index_md] == 1) {
        ...
        if(pba->has_ncdm || pba->has_ddec) output_cb_tot[index_k] = psp->ln_pk_cb[index_k];
      }
      else {
  ...
  if (psp->ic_size[index_md] > 1) {
    for (index_k=0; index_k<psp->ln_k_size; index_k++) {
      ...
      if (pba->has_ncdm || pba->has_ddec) output_cb_tot[index_k] = 0.;
      for (index_ic1=0; index_ic1 < psp->ic_size[index_md]; index_ic1++) {
        for (index_ic2 = index_ic1; index_ic2 < psp->ic_size[index_md]; index_ic2++) {
          index_ic1_ic2 = index_symmetric_matrix(index_ic1,index_ic2,psp->ic_size[index_md]);
          if (index_ic1 == index_ic2) {
            output_tot[index_k] += exp(output_ic[index_k * psp->ic_ic_size[index_md] + index_ic1_ic2]);
            if(pba->has_ncdm || pba->has_ddec) output_cb_tot[index_k] += exp(output_cb_ic[index_k * psp->ic_ic_size[index_md] + index_ic1_ic2]);
          }
          else {
            if (psp->is_non_zero[index_md][index_ic1_ic2] == _TRUE_) {
              ...
              if(pba->has_ncdm || pba->has_ddec) output_cb_ic[index_k * psp->ic_ic_size[index_md] + index_ic1_ic2] = 0.;
            }
          }
        }
      }
  ...
  if (mode == linear) {
    ...
    if (psp->ic_size[index_md] == 1) {
      for (index_k=0; index_k<psp->ln_k_size; index_k++) {
        ...
        if(pba->has_ncdm || pba->has_ddec) output_cb_tot[index_k] = exp(output_cb_tot[index_k]);
      }
    }
  ...
  /** - --> (b) logarithmic mode: if only one initial condition, nothing to be done; if several initial conditions, convert output_tot to logarithmic format, output_ic is already in this format */
    ...
    if (psp->ic_size[index_md] > 1) {
      for (index_k=0; index_k<psp->ln_k_size; index_k++) {
        ...
        if(pba->has_ncdm || pba->has_ddec) output_cb_tot[index_k] = log(output_cb_tot[index_k]);
      }
    }
  }
...
int spectra_pk_at_k_and_z(
  ...
  if (k < exp(psp->ln_k[0])) {
    ...
    if (k == 0.) {
      if (psp->ic_size[index_md] == 1) {
        *pk_tot=0.;
        if (pba->has_ncdm || pba->has_ddec) *pk_cb_tot=0.;
      }
      else {
        for (index_ic1_ic2 = 0; index_ic1_ic2 < psp->ic_ic_size[index_md]; index_ic1_ic2++) {
          pk_ic[index_ic1_ic2] = 0.;
          if (pba->has_ncdm || pba->has_ddec) pk_cb_ic[index_ic1_ic2] = 0.;
        }
      }
    }
  ...
  /** - deal with case kmin <= k <= kmax */
    ...
    if (psp->ic_size[index_md] == 1) {
      ...
      *pk_tot = exp(*pk_tot);
      ...
      for (index_ic1 = 0; index_ic1 < psp->ic_size[index_md]; index_ic1++) {
        ...
        if(pba->has_ncdm || pba->has_ddec) pk_cb_ic[index_ic1_ic2] = exp(pk_cb_ic[index_ic1_ic2]);
      }
      for (index_ic1 = 0; index_ic1 < psp->ic_size[index_md]; index_ic1++) {
        for (index_ic2 = index_ic1+1; index_ic2 < psp->ic_size[index_md]; index_ic2++) {
          index_ic1_ic2 = index_symmetric_matrix(index_ic1,index_ic2,psp->ic_size[index_md]);
    ...
    free(spectrum_cb_at_z);
    free(spline);
    if(pba->has_ncdm || pba->has_ddec) free(spline_cb);
  }
        ...
        if (psp->is_non_zero[index_md][index_ic1_ic2] == _TRUE_) {
          ...
          if (index_ic1 == index_ic2){
            *pk_tot += pk_ic[index_ic1_ic2];
            if(pba->has_ncdm || pba->has_ddec) *pk_cb_tot += pk_cb_ic[index_ic1_ic2];
          }
          else{
            *pk_tot += 2.*pk_ic[index_ic1_ic2];
            if(pba->has_ncdm || pba->has_ddec) *pk_cb_tot += 2.*pk_cb_ic[index_ic1_ic2];
          }
        }
        else {
          pk_ic[index_ic1_ic2] = 0.;
...
int spectra_pk_nl_at_z(
  ...
  if (psp->ln_tau_size == 1) {
    ...
    for (index_k=0; index_k<psp->ln_k_size; index_k++) {
      ...
      if (pba->has_ncdm || pba->has_ddec) output_cb_tot[index_k] = psp->ln_pk_cb_nl[index_k];
    }
  }
...
int spectra_indices(
  ...
  class_define_index(psp->index_tr_delta_ddec1,ppt->has_source_delta_ddec,index_tr,pba->N_ddec);
  ...
  class_define_index(psp->index_tr_theta_ddec1,ppt->has_source_theta_ddec,index_tr,pba->N_ddec);
...
int spectra_pk(
  ...
  for (index_tau=0 ; index_tau < psp->ln_tau_size; index_tau++) {
    for (index_k=0; index_k<psp->ln_k_size; index_k++) {
      ...
      if(pba->has_ncdm || pba->has_ddec) ln_pk_cb_tot = log(pk_cb_tot);

      psp->ln_pk_l[index_tau * psp->ln_k_size + index_k] = ln_pk_tot;

      if(pba->has_ncdm || pba->has_ddec) psp->ln_pk_cb_l[index_tau * psp->ln_k_size + index_k] = ln_pk_cb_tot;
      ...
      if((pba->has_ncdm || pba->has_ddec) && (pnl->method != nl_none) && (index_tau >= delta_index_nl_cb)){
      ...
      for (index_ic = 0; index_ic < psp->ic_size[index_md]; index_ic++) {
        ...
        if (pba->has_ddec == _TRUE_) {
          ...
          for (n_ddec=0; n_ddec < pba->N_ddec; n_ddec++) {
            ...
            rho_i = pvecback_sp_long[pba->index_bg_rho_ddec1+n_ddec];

            if (ppt->has_source_delta_ddec == _TRUE_) {

              delta_i = ppt->sources[index_md]
                [index_ic * ppt->tp_size[index_md] + ppt->index_tp_delta_ddec1+n_ddec]
                [(index_tau-psp->ln_tau_size+ppt->tau_size) * ppt->k_size[index_md] + index_k];

              psp->matter_transfer[((index_tau*psp->ln_k_size + index_k) * psp->ic_size[index_md] + index_ic) * psp->tr_size + psp->index_tr_delta_ddec1+n_ddec] = delta_i;
            ...
            if (ppt->has_source_theta_ddec == _TRUE_) {
              ...
              theta_i = ppt->sources[index_md]
                [index_ic * ppt->tp_size[index_md] + ppt->index_tp_theta_ddec1+n_ddec]
                [(index_tau-psp->ln_tau_size+ppt->tau_size) * ppt->k_size[index_md] + index_k];

              psp->matter_transfer[((index_tau*psp->ln_k_size + index_k) * psp->ic_size[index_md] + index_ic) * psp->tr_size + psp->index_tr_theta_ddec1+n_ddec] = theta_i;

              rho_plus_p_theta_tot += (rho_i + pvecback_sp_long[pba->index_bg_p_ddec1+n_ddec]) * theta_i;

            }

            rho_plus_p_tot += (rho_i + pvecback_sp_long[pba->index_bg_p_ddec1+n_ddec]);
    ...
    for (index_k=0; index_k<psp->ln_k_size; index_k++) {
      ...
      if (output_format == class_format) {
        ...
        if (ppt->has_density_transfers == _TRUE_) {
          ...
          if (pba->has_ddec == _TRUE_){
            for (n_ddec = 0; n_ddec < pba->N_ddec; n_ddec++){
              class_store_double(dataptr,tk[psp->index_tr_delta_ddec1+n_ddec],ppt->has_source_delta_ddec,storeidx);
            }
          }
        ...
        if (ppt->has_velocity_transfers == _TRUE_) {
          ...
          if (pba->has_ddec == _TRUE_){
            for (n_ddec = 0; n_ddec < pba->N_ddec; n_ddec++){
              class_store_double(dataptr,tk[psp->index_tr_theta_ddec1+n_ddec],ppt->has_source_theta_ddec,storeidx);
            }
          }
...
int spectra_fast_pk_at_kvec_and_zvec(
  ...
  for(index_k = 0; index_k<kvec_size; index_k++){
    ...
    for (index_z = 0; index_z < zvec_size; index_z++) {
      ...
      if(pba->has_ncdm || pba->has_ddec) pk_cb_tot_out[index_z*kvec_size+index_k] = 0.;
    }
