...
int background_functions(
  ...
  int n_ncdm;
  /* background ddec quantities */
  double rho_ddec,p_ddec,pseudo_p_ddec;
  /* index for n_ddec species */
  int n_ddec;
  ...
  if (pba->has_ddec == _TRUE_) {
    ...
    for(n_ddec=0; n_ddec<pba->N_ddec; n_ddec++){
      ...
      /* function returning background ddec[n_ddec] quantities (only
         those for which non-NULL pointers are passed) */
      class_call(background_ddec_momenta(
                                         pba->q_ddec_bg[n_ddec],
                                         pba->w_ddec_bg[n_ddec],
                                         pba->q_size_ddec_bg[n_ddec],
                                         pba->M_ddec[n_ddec],
                                         pba->factor_ddec[n_ddec],
                                         1./a_rel-1.,
                                         NULL,
                                         &rho_ddec,
                                         &p_ddec,
      ...
      pvecback[pba->index_bg_rho_ddec1+n_ddec] = rho_ddec;
      rho_tot += rho_ddec;
      pvecback[pba->index_bg_p_ddec1+n_ddec] = p_ddec;
      p_tot += p_ddec;
      pvecback[pba->index_bg_pseudo_p_ddec1+n_ddec] = pseudo_p_ddec;

      /* (3 p_ddec1) is the "relativistic" contribution to rho_ddec1 */
      rho_r += 3.* p_ddec;

      /* (rho_ddec1 - 3 p_ddec1) is the "non-relativistic" contribution
         to rho_ddec1 */
      rho_m += rho_ddec - 3.* p_ddec;
    }
  }
...
int background_init(
  ...
  if (pba->background_verbose > 0) {
    ...
    if (pba->N_ncdm > 0) {
      ...
      // For ddec we had to move this line outside the if statement
      //Neff = pba->Omega0_ur/7.*8./pow(4./11.,4./3.)/pba->Omega0_g;
    ...
    /* below we want to inform the user about ddec species*/
    if (pba->N_ddec > 0) {

      // for ddec we want the next line commented so we sum up all Neff contributions
      //Neff = pba->Omega0_ur/7.*8./pow(4./11.,4./3.)/pba->Omega0_g;

      /* loop over ddec species */
      for (n_ddec=0;n_ddec<pba->N_ddec; n_ddec++) {

        /* inform if p-s-d read in files */
        if (pba->got_files[n_ddec] == _TRUE_) {
          printf(" -> ddec species i=%d read from file %s\n",n_ddec+1,pba->ddec_psd_files+filenum*_ARGUMENT_LENGTH_MAX_);
          filenum++;
        }

        /* call this function to get rho_ddec */
        background_ddec_momenta(pba->q_ddec_bg[n_ddec],
                                pba->w_ddec_bg[n_ddec],
                                pba->q_size_ddec_bg[n_ddec],
                                0.,
                                pba->factor_ddec[n_ddec],
        ...
        /* inform user of the contribution of each species to
           radiation density (in relativistic limit): should be
           between 1.01 and 1.02 for each active neutrino species;
           evaluated as rho_ddec/rho_nu_rel where rho_nu_rel is the
           density of one neutrino in the instantaneous decoupling
           limit, i.e. assuming T_nu=(4/11)^1/3 T_gamma (this comes
           from the definition of N_eff) */
        rho_nu_rel = 56.0/45.0*pow(_PI_,6)*pow(4.0/11.0,4.0/3.0)*_G_/pow(_h_P_,3)/pow(_c_,7)*
        ...
        printf(" -> ddec species i=%d sampled with %d (resp. %d) points for purpose of background (resp. perturbation) integration. In the relativistic limit it gives Delta N_eff = %g\n",
               n_ddec+1,
               pba->q_size_ddec_bg[n_ddec],
               pba->q_size_ddec[n_ddec],
               rho_ddec_rel/rho_nu_rel);

        Neff += rho_ddec_rel/rho_nu_rel;

      }

      printf(" -> total N_eff = %g (summed over ultra-relativistic, ncdm and ddec species)\n",Neff);
  ...
  /* in verbose mode, inform the user about the value of the ddec
     masses in eV and about the ratio [m/omega_ddec] in eV (the usual
     93 point something). Probably should not be 93 for ddec*/
  if ((pba->background_verbose > 0) && (pba->has_ddec == _TRUE_)) {
    for (n_ddec=0; n_ddec < pba->N_ddec; n_ddec++) {
      printf(" -> non-cold dark matter species (ddec) with i=%d has m_i = %e eV (so m_i / omega_i =%e eV)\n",
             n_ddec+1,
             pba->m_ddec_in_eV[n_ddec],
             pba->m_ddec_in_eV[n_ddec]*pba->deg_ddec[n_ddec]/pba->Omega0_ddec[n_ddec]/pba->h/pba->h);
    }
  }
...
int background_free_input(
  ...
  if (pba->Omega0_ddec_tot != 0.){
    for(k=0; k<pba->N_ddec; k++){
      free(pba->q_ddec[k]);
      free(pba->w_ddec[k]);
      free(pba->q_ddec_bg[k]);
      free(pba->w_ddec_bg[k]);
      free(pba->dlnf0_dlnq_ddec[k]);
    }
    free(pba->ddec_quadrature_strategy);
    free(pba->ddec_input_q_size);
    free(pba->ddec_qmax);
    free(pba->q_ddec);
    free(pba->w_ddec);
    free(pba->q_ddec_bg);
    free(pba->w_ddec_bg);
    free(pba->dlnf0_dlnq_ddec);
    free(pba->q_size_ddec);
    free(pba->q_size_ddec_bg);
    free(pba->M_ddec);
    free(pba->T_ddec);
    free(pba->ksi_ddec);
    free(pba->deg_ddec);
    free(pba->Omega0_ddec);
    // ddec specific start
    //free(pba->zdec_ddec);
    //free(pba->dzdec_ddec);
    // ddec end
    free(pba->m_ddec_in_eV);
    free(pba->factor_ddec);
    //if(pba->got_files!=NULL)
    //  free(pba->got_files);
    if(pba->ddec_psd_files!=NULL)
      free(pba->ddec_psd_files);
    if(pba->ddec_psd_parameters!=NULL)
      free(pba->ddec_psd_parameters);
  }

  if ((pba->Omega0_ncdm_tot != 0.) || (pba->Omega0_ddec_tot != 0.)){
    if(pba->got_files!=NULL)
      free(pba->got_files);
  }
...
int background_indices(
  ...
  /* - indices for ddec. We only define the indices for ddec1
     (density, pressure, pseudo-pressure), the other ddec indices
     are contiguous */
  class_define_index(pba->index_bg_rho_ddec1,pba->has_ddec,index_bg,pba->N_ddec);
  class_define_index(pba->index_bg_p_ddec1,pba->has_ddec,index_bg,pba->N_ddec);
  class_define_index(pba->index_bg_pseudo_p_ddec1,pba->has_ddec,index_bg,pba->N_ddec);
...
/**
 * This is the routine where the distribution function f0(q) of each
 * ddec species is specified (it is the only place to modify if you
...
int background_ddec_distribution(
                                 void * pbadist,
                                 double q,
                                 double * f0
                                 ) {
  ...
  pbadist_local = pbadist;          /* restore actual format of pbadist */
  pba = pbadist_local->pba;         /* extract the background structure from it */
  param = pba->ddec_psd_parameters; /* extract the optional parameter list from it */
  n_ddec = pbadist_local->n_ddec;   /* extract index of ddec species under consideration */
  ksi = pba->ksi_ddec[n_ddec];      /* extract chemical potential */
  ...
  /** - b) deal now with case of reading analytical function */
    ...
    /**
       Next enter your analytic expression(s) for the p.s.d.'s. If
       you need different p.s.d.'s for different species, put each
       p.s.d inside a condition, like for instance: if (n_ddec==2)
       {*f0=...}.  Remember that n_ddec = 0 refers to the first
       species.
    */
    ...
    /**************************************************/
    // Fermi-Dirac
    if (pba->ddec_fermion_boson[n_ddec]==0){
      *f0 = 1.0/pow(2*_PI_,3)*(1./(exp(q-ksi)+1.) +1./(exp(q+ksi)+1.));
    }
    // Bose-Einstein
    else {
    ...
    if (_FALSE_) {
      ...
      /* We must use the list of extra parameters read in input, stored in the
         ddec_psd_parameter list, extracted above from the structure
         and now called param[..] */

      /* check that this list has been read */
      class_test(param == NULL,
                 pba->error_message,
                 "Analytic expression wants to use 'ddec_psd_parameters', but they have not been entered!");
      ...
      *f0=0.0;
      for(i=0;i<3;i++){
        ...
        *f0 += mixing_matrix[i][n_ddec]*1.0/pow(2*_PI_,3)*(1./(exp(q-pba->ksi_ddec[i])+1.) +1./(exp(q+pba->ksi_ddec[i])+1.));
...
int background_ddec_test_function(
                                  void * pbadist,
                                  double q,
                                  double * test
                                  ) {
...
/**
 * This function finds optimal quadrature weights for each ddec
 * species
 *
 * @param ppr Input: precision structure
 * @param pba Input/Output: background structure
...
int background_ddec_init(
  ...
  /* Allocate pointer arrays: */
  class_alloc(pba->q_ddec, sizeof(double*)*pba->N_ddec,pba->error_message);
  class_alloc(pba->w_ddec, sizeof(double*)*pba->N_ddec,pba->error_message);
  class_alloc(pba->q_ddec_bg, sizeof(double*)*pba->N_ddec,pba->error_message);
  class_alloc(pba->w_ddec_bg, sizeof(double*)*pba->N_ddec,pba->error_message);
  class_alloc(pba->dlnf0_dlnq_ddec, sizeof(double*)*pba->N_ddec,pba->error_message);

  /* Allocate pointers: */
  class_alloc(pba->q_size_ddec,sizeof(int)*pba->N_ddec,pba->error_message);
  class_alloc(pba->q_size_ddec_bg,sizeof(int)*pba->N_ddec,pba->error_message);
  class_alloc(pba->factor_ddec,sizeof(double)*pba->N_ddec,pba->error_message);

  for(k=0, filenum=0; k<pba->N_ddec; k++){
    pbadist.n_ddec = k;
    pbadist.q = NULL;
    pbadist.tablesize = 0;
    /*Do we need to read in a file to interpolate the distribution function? */
    if ((pba->got_files!=NULL)&&(pba->got_files[k]==_TRUE_)){
      psdfile = fopen(pba->ddec_psd_files+filenum*_ARGUMENT_LENGTH_MAX_,"r");
      class_test(psdfile == NULL,pba->error_message,
                 "Could not open file %s!",pba->ddec_psd_files+filenum*_ARGUMENT_LENGTH_MAX_);
      // Find size of table:
      for (row=0,status=2; status==2; row++){
        status = fscanf(psdfile,"%lf %lf",&tmp1,&tmp2);
      }
    ...
    /* Handle perturbation qsampling: */
    if (pba->ddec_quadrature_strategy[k]==qm_auto){
      /** Automatic q-sampling for this species */
      class_alloc(pba->q_ddec[k],_QUADRATURE_MAX_*sizeof(double),pba->error_message);
      class_alloc(pba->w_ddec[k],_QUADRATURE_MAX_*sizeof(double),pba->error_message);

      class_call(get_qsampling(pba->q_ddec[k],
			       pba->w_ddec[k],
			       &(pba->q_size_ddec[k]),
			       _QUADRATURE_MAX_,
			       ppr->tol_ddec,
			       pbadist.q,
			       pbadist.tablesize,
			       background_ddec_test_function,
			       background_ddec_distribution,
			       &pbadist,
			       pba->error_message),
		 pba->error_message,
		 pba->error_message);
      pba->q_ddec[k]=realloc(pba->q_ddec[k],pba->q_size_ddec[k]*sizeof(double));
      pba->w_ddec[k]=realloc(pba->w_ddec[k],pba->q_size_ddec[k]*sizeof(double));


      if (pba->background_verbose > 0)
	printf("ddec species i=%d sampled with %d points for purpose of perturbation integration\n",
	       k+1,
	       pba->q_size_ddec[k]);

      /* Handle background q_sampling: */
      class_alloc(pba->q_ddec_bg[k],_QUADRATURE_MAX_BG_*sizeof(double),pba->error_message);
      class_alloc(pba->w_ddec_bg[k],_QUADRATURE_MAX_BG_*sizeof(double),pba->error_message);

      class_call(get_qsampling(pba->q_ddec_bg[k],
			       pba->w_ddec_bg[k],
			       &(pba->q_size_ddec_bg[k]),
			       _QUADRATURE_MAX_BG_,
			       ppr->tol_ddec_bg,
			       pbadist.q,
			       pbadist.tablesize,
      ...
      pba->q_ddec_bg[k]=realloc(pba->q_ddec_bg[k],pba->q_size_ddec_bg[k]*sizeof(double));
      pba->w_ddec_bg[k]=realloc(pba->w_ddec_bg[k],pba->q_size_ddec_bg[k]*sizeof(double));

      /** - in verbose mode, inform user of number of sampled momenta
	  for background quantities */
      if (pba->background_verbose > 0)
	printf("ddec species i=%d sampled with %d points for purpose of background integration\n",
	       k+1,
	       pba->q_size_ddec_bg[k]);
    }
    else{
      /** Manual q-sampling for this species. Same sampling used for both perturbation and background sampling, since this will usually be a high precision setting anyway */
      pba->q_size_ddec_bg[k] = pba->ddec_input_q_size[k];
      pba->q_size_ddec[k] = pba->ddec_input_q_size[k];
      class_alloc(pba->q_ddec_bg[k],pba->q_size_ddec_bg[k]*sizeof(double),pba->error_message);
      class_alloc(pba->w_ddec_bg[k],pba->q_size_ddec_bg[k]*sizeof(double),pba->error_message);
      class_alloc(pba->q_ddec[k],pba->q_size_ddec[k]*sizeof(double),pba->error_message);
      class_alloc(pba->w_ddec[k],pba->q_size_ddec[k]*sizeof(double),pba->error_message);
      class_call(get_qsampling_manual(pba->q_ddec[k],
				      pba->w_ddec[k],
				      pba->q_size_ddec[k],
				      pba->ddec_qmax[k],
				      pba->ddec_quadrature_strategy[k],
				      pbadist.q,
				      pbadist.tablesize,
				      background_ddec_distribution,
				      &pbadist,
		 ...
		 pba->error_message,
		 pba->error_message);
      for (index_q=0; index_q<pba->q_size_ddec[k]; index_q++) {
	pba->q_ddec_bg[k][index_q] = pba->q_ddec[k][index_q];
	pba->w_ddec_bg[k][index_q] = pba->w_ddec[k][index_q];
      }
    /** - in verbose mode, inform user of number of sampled momenta
        for background quantities */
      if (pba->background_verbose > 0)
	printf("ddec species i=%d sampled with %d points for purpose of background andperturbation integration using the manual method\n",
	       k+1,
	       pba->q_size_ddec[k]);
    }

    class_alloc(pba->dlnf0_dlnq_ddec[k],
                pba->q_size_ddec[k]*sizeof(double),
                pba->error_message);


    for (index_q=0; index_q<pba->q_size_ddec[k]; index_q++) {
      q = pba->q_ddec[k][index_q];
      class_call(background_ddec_distribution(&pbadist,q,&f0),
                 pba->error_message,pba->error_message);
      ...
      for(tolexp=_PSD_DERIVATIVE_EXP_MIN_; tolexp<_PSD_DERIVATIVE_EXP_MAX_; tolexp++){
        ...
        if (index_q == 0){
          dq = MIN((0.5-ppr->smallest_allowed_variation)*q,2*exp(tolexp)*(pba->q_ddec[k][index_q+1]-q));
        }
        else if (index_q == pba->q_size_ddec[k]-1){
          dq = exp(tolexp)*2.0*(pba->q_ddec[k][index_q]-pba->q_ddec[k][index_q-1]);
        }
        else{
          dq = exp(tolexp)*(pba->q_ddec[k][index_q+1]-pba->q_ddec[k][index_q-1]);
        }

        class_call(background_ddec_distribution(&pbadist,q-2*dq,&f0m2),
                   pba->error_message,pba->error_message);
        class_call(background_ddec_distribution(&pbadist,q+2*dq,&f0p2),
                   pba->error_message,pba->error_message);

        if (fabs((f0p2-f0m2)/f0)>sqrt(ppr->smallest_allowed_variation)) break;
      }

      class_call(background_ddec_distribution(&pbadist,q-dq,&f0m1),
                 pba->error_message,pba->error_message);
      class_call(background_ddec_distribution(&pbadist,q+dq,&f0p1),
                 pba->error_message,pba->error_message);
      //5 point estimate of the derivative:
      df0dq = (+f0m2-8*f0m1+8*f0p1-f0p2)/12.0/dq;
      //printf("df0dq[%g] = %g. dlf=%g ?= %g. f0 =%g.\n",q,df0dq,q/f0*df0dq,
      //Avoid underflow in extreme tail:
      if (fabs(f0)==0.)
        pba->dlnf0_dlnq_ddec[k][index_q] = -q; /* valid for whatever f0 with exponential tail in exp(-q) */
      else
        pba->dlnf0_dlnq_ddec[k][index_q] = q/f0*df0dq;
    }

    pba->factor_ddec[k]=pba->deg_ddec[k]*4*_PI_*pow(pba->T_cmb*pba->T_ddec[k]*_k_B_,4)*8*_PI_*_G_
      /3./pow(_h_P_/2./_PI_,3)/pow(_c_,7)*_Mpc_over_m_*_Mpc_over_m_;
...
/**
 * For a given ddec species: given the quadrature weights, the mass
 * and the redshift, find background quantities by a quick weighted
 * sum over.  Input parameters passed as NULL pointers are not
 * evaluated for speed-up
 *
...
int background_ddec_momenta(
                            ...
                            double * p,   // pressure
                            double * drho_dM,  // d rho / d M used in next function
                            double * pseudo_p  // pseudo-p used in ddec fluid approx
                            ) {
...
/**
 * When the user passed the density fraction Omega_ddec or
 * omega_ddec in input but not the mass, infer the mass with Newton iteration method.
 *
 * @param ppr    Input: precision structure
 * @param pba    Input/Output: background structure
 * @param n_ddec Input: index of ddec species
 */

int background_ddec_M_from_Omega(
                                 struct precision *ppr,
                                 struct background *pba,
                                 int n_ddec
                                 ) {
  ...
  rho0 = pba->H0*pba->H0*pba->Omega0_ddec[n_ddec]; /*Remember that rho is defined such that H^2=sum(rho_i) */
  M = 0.0;
  ...
  class_test(rho0<rho,pba->error_message,
             "The value of Omega for the %dth species, %g, is less than for a massless species! It should be atleast %g. Check your input.",
             n_ddec,pba->Omega0_ddec[n_ddec],pba->Omega0_ddec[n_ddec]*rho/rho0);
  ...
  for (iter=1; iter<=maxiter; iter++){
    ...
    if ((M+deltaM)<0.0) deltaM = -M/2.0; /* Avoid overshooting to negative M value. */
    M += deltaM; /* Update value of M.. */
    if (fabs(deltaM/M)<ppr->tol_M_ddec){
      /* Accuracy reached.. */
      pba->M_ddec[n_ddec] = M;
      break;
    }
...
int background_initial_conditions(
  ...
  double rho_ddec, p_ddec, rho_ddec_rel_tot=0.;
  ...
  */
  ...
  /**  If we have ddec species, perhaps we need to start earlier
      than the standard value for the species to be relativistic.
      This could happen for some WDM models.
  */
  ...
  if (pba->has_ddec == _TRUE_) {
	...
	class_call(background_ddec_momenta(pba->q_ddec_bg[n_ddec],
					   pba->w_ddec_bg[n_ddec],
					   pba->q_size_ddec_bg[n_ddec],
					   pba->M_ddec[n_ddec],
					   pba->factor_ddec[n_ddec],
                   ...
                   pba->error_message,
                   pba->error_message);
	rho_ddec_rel_tot += 3.*p_ddec;
	if (fabs(p_ddec/rho_ddec-1./3.)>ppr->tol_ddec_initial_w)
	  is_early_enough = _FALSE_;
      }
      if (is_early_enough == _TRUE_)
	break;
    ...
    class_test(counter == _MAX_IT_,
	       pba->error_message,
	       "Search for initial scale factor a such that all ddec species are relativistic failed.");
  }
  ...
  if (pba->has_ddec == _TRUE_){
    /** - We must add the relativistic contribution from ddec species */
    rho_rad += rho_ddec_rel_tot;
  }
  if (pba->has_dcdm == _TRUE_){
...
int background_output_titles(struct background * pba,
  ...
  if (pba->has_ddec == _TRUE_){
    for (n=0; n<pba->N_ddec; n++){
      sprintf(tmp,"(.)rho_ddec[%d]",n);
      class_store_columntitle(titles,tmp,_TRUE_);
      sprintf(tmp,"(.)p_ddec[%d]",n);
      class_store_columntitle(titles,tmp,_TRUE_);
    }
  }
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++){
    ...
    if (pba->has_ddec == _TRUE_){
      for (n=0; n<pba->N_ddec; n++){
        class_store_double(dataptr,pvecback[pba->index_bg_rho_ddec1+n],_TRUE_,storeidx);
        class_store_double(dataptr,pvecback[pba->index_bg_p_ddec1+n],_TRUE_,storeidx);
      }
    }
