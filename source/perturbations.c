...
int perturb_init(
  ...
  /* for printing ddec horizon scale */
  //double tau_ddec, k_horizon;
  //int n_ddec;
  ...
  if (pba->has_ddec == _TRUE_) {

    class_test ((ppr->ddec_fluid_approximation < ddecfa_mb) ||
                (ppr->ddec_fluid_approximation > ddecfa_none),
                ppt->error_message,
                "your ddec_fluid_approximation is set to %d, out of range defined in perturbations.h",ppr->ddec_fluid_approximation);
  ...
  /*
    for (n_ddec=0; n_ddec < pba->N_ddec; n_ddec++){
      // print horizon scale at dark decoupling //
      if (pba->zdec_ddec[n_ddec] >= 0.) {
        class_call(background_tau_of_z(pba,
                                       pba->zdec_ddec[n_ddec],
                                       &tau_ddec),
                   pba->error_message,
                   ppt->error_message);
        k_horizon = pow(tau_ddec,-1);
        fprintf(stdout,"k_horizon = %e\n",k_horizon);
      }
    }
  */
  }
  ...
  if (ppt->has_tensors == _TRUE_) {
    ...
    ppt->evolve_tensor_ddec = _FALSE_;
...
int perturb_indices_of_perturbs(
  ...
  ppt->has_source_delta_ddec = _FALSE_;
  ...
  ppt->has_source_theta_ddec = _FALSE_;
  ...
  for (index_md = 0; index_md < ppt->md_size; index_md++) {
    ...
    if (_scalars_) {
      ...
      if (ppt->has_density_transfers == _TRUE_) {
        ...
        if (pba->has_ddec == _TRUE_)
          ppt->has_source_delta_ddec = _TRUE_;
      ...
      if (ppt->has_velocity_transfers == _TRUE_) {
        ...
        if (pba->has_ddec == _TRUE_)
          ppt->has_source_theta_ddec = _TRUE_;
      }
      ...
      class_define_index(ppt->index_tp_delta_ddec1,ppt->has_source_delta_ddec,index_type,pba->N_ddec);
      ...
      class_define_index(ppt->index_tp_theta_ddec1,ppt->has_source_theta_ddec,index_type,pba->N_ddec);
...
int perturb_workspace_init(
  ...
  if (_scalars_) {
    ...
    if (pba->has_ddec == _TRUE_) ppw->max_l_max = MAX(ppw->max_l_max, ppr->l_max_ddec);
  ...
  if (_scalars_) {
    ...
    class_define_index(ppw->index_ap_ddecfa,pba->has_ddec,index_ap,1);
  ...
  if (_scalars_) {
    ...
    if (pba->has_ddec == _TRUE_) {
      ppw->approx[ppw->index_ap_ddecfa]=(int)ddecfa_off;
    }
  }
  ...
  if (_scalars_) {
    ...
    if ((ppt->has_density_transfers == _TRUE_) || (ppt->has_velocity_transfers == _TRUE_) || (ppt->has_source_delta_m == _TRUE_)) {
      ...
      class_alloc(ppw->delta_ddec,pba->N_ddec*sizeof(double),ppt->error_message);
      class_alloc(ppw->theta_ddec,pba->N_ddec*sizeof(double),ppt->error_message);
      class_alloc(ppw->shear_ddec,pba->N_ddec*sizeof(double),ppt->error_message);
...
int perturb_solve(
  ...
  if (pba->has_ddec == _TRUE_) {
    for (n_ddec=0; n_ddec < pba->N_ddec; n_ddec++) {
      class_test(fabs(ppw->pvecback[pba->index_bg_p_ddec1+n_ddec]/ppw->pvecback[pba->index_bg_rho_ddec1+n_ddec]-1./3.)>ppr->tol_ddec_initial_w,
                 ppt->error_message,
                 "your choice of initial time for integrating wavenumbers is inappropriate: it corresponds to a time at which the ddec species number %d is not ultra-relativistic anymore, with w=%g, p=%g and rho=%g\n",
                 n_ddec,
                 ppw->pvecback[pba->index_bg_p_ddec1+n_ddec]/ppw->pvecback[pba->index_bg_rho_ddec1+n_ddec],
                 ppw->pvecback[pba->index_bg_p_ddec1+n_ddec],
                 ppw->pvecback[pba->index_bg_rho_ddec1+n_ddec]);
    }
  }
  ...
  while ((tau_upper - tau_lower)/tau_lower > ppr->tol_tau_approx) {
    ...
    /* if there are non-cold relics (ddec), check that they are relativistic enough */
    if (pba->has_ddec == _TRUE_) {
      for (n_ddec=0; n_ddec < pba->N_ddec; n_ddec++) {
        if (fabs(ppw->pvecback[pba->index_bg_p_ddec1+n_ddec]/ppw->pvecback[pba->index_bg_rho_ddec1+n_ddec]-1./3.) > ppr->tol_ddec_initial_w)
          is_early_enough = _FALSE_;
      }
    }
...
int perturb_prepare_output(struct background * pba,
  ...
  if (ppt->k_output_values_num > 0) {
    ...
    if (ppt->has_scalars == _TRUE_){
      ...
      /* Non-cold dark matter (ddec) */
      if ((pba->has_ddec == _TRUE_) && ((ppt->has_density_transfers == _TRUE_) || (ppt->has_velocity_transfers == _TRUE_) || (ppt->has_source_delta_m == _TRUE_))) {
        for(n_ddec=0; n_ddec < pba->N_ddec; n_ddec++){
          sprintf(tmp,"delta_ddec[%d]",n_ddec);
          class_store_columntitle(ppt->scalar_titles,tmp,_TRUE_);
          sprintf(tmp,"theta_ddec[%d]",n_ddec);
          class_store_columntitle(ppt->scalar_titles,tmp,_TRUE_);
          sprintf(tmp,"shear_ddec[%d]",n_ddec);
          class_store_columntitle(ppt->scalar_titles,tmp,_TRUE_);
          sprintf(tmp,"cs2_ddec[%d]",n_ddec);
          class_store_columntitle(ppt->scalar_titles,tmp,_TRUE_);
        }
      }
    ...
    if (ppt->has_tensors == _TRUE_){
      ...
      if (ppt->evolve_tensor_ddec == _TRUE_) {
        for(n_ddec=0; n_ddec < pba->N_ddec; n_ddec++){
          sprintf(tmp,"delta_ddec[%d]",n_ddec);
          class_store_columntitle(ppt->tensor_titles,tmp,_TRUE_);
          sprintf(tmp,"theta_ddec[%d]",n_ddec);
    ...
    for (index_switch=1; index_switch<interval_number; index_switch++) {
      ...
      if (ppt->perturbations_verbose>2) {
        ...
        if (_scalars_) {
          ...
          if (pba->has_ddec == _TRUE_) {
            if ((interval_approx[index_switch-1][ppw->index_ap_ddecfa]==(int)ddecfa_off) &&
                (interval_approx[index_switch][ppw->index_ap_ddecfa]==(int)ddecfa_on)) {
              fprintf(stdout,"Mode k=%e: will switch on ddec fluid approximation at tau=%e\n",k,interval_limit[index_switch]);
            }
          }
        }
...
int perturb_vector_init(
  ...
  int n_ddec,ddec_l_size;
  double rho_plus_p_ncdm,q,q2,epsilon,a,factor;
  double rho_plus_p_ddec;
  ...
  if (_scalars_) {
    ...
    if (pba->has_ddec == _TRUE_) {
      ppv->index_pt_psi0_ddec1 = index_pt; /* density of ultra-relativistic neutrinos/relics */
      ppv->N_ddec = pba->N_ddec;
      class_alloc(ppv->l_max_ddec,ppv->N_ddec*sizeof(double),ppt->error_message);
      class_alloc(ppv->q_size_ddec,ppv->N_ddec*sizeof(double),ppt->error_message);

      for(n_ddec = 0; n_ddec < pba->N_ddec; n_ddec++){
        // Set value of ppv->l_max_ddec:
        if(ppw->approx[ppw->index_ap_ddecfa] == (int)ddecfa_off){
          /* reject inconsistent values of the number of mutipoles in ultra relativistic neutrino hierarchy */
          class_test(ppr->l_max_ddec < 4,
                     ppt->error_message,
                     "ppr->l_max_ddec=%d should be at least 4, i.e. we must integrate at least over first four momenta of non-cold dark matter (ddec) perturbed phase-space distribution",n_ddec);
          //Copy value from precision parameter:
          ppv->l_max_ddec[n_ddec] = ppr->l_max_ddec;
          ppv->q_size_ddec[n_ddec] = pba->q_size_ddec[n_ddec];
        }
        else{
          // In the fluid approximation, hierarchy is cut at lmax = 2 and q dependence is integrated out:
          ppv->l_max_ddec[n_ddec] = 2;
          ppv->q_size_ddec[n_ddec] = 1;
        }
        index_pt += (ppv->l_max_ddec[n_ddec]+1)*ppv->q_size_ddec[n_ddec];
      }
    }
  ...
  if (_tensors_) {
    ...
    if (ppt->evolve_tensor_ddec == _TRUE_) {
      ppv->index_pt_psi0_ddec1 = index_pt;
      ppv->N_ddec = pba->N_ddec;
      class_alloc(ppv->l_max_ddec,ppv->N_ddec*sizeof(double),ppt->error_message);
      class_alloc(ppv->q_size_ddec,ppv->N_ddec*sizeof(double),ppt->error_message);
  ...
  if (_scalars_) {
    ...
    if (pba->has_ddec == _TRUE_) {
      ...
      /* we don't need ddec multipoles above l=2 (but they are
         defined only when ddecfa is off) */

      index_pt = ppv->index_pt_psi0_ddec1;
      for(n_ddec = 0; n_ddec < ppv-> N_ddec; n_ddec++){
        for(index_q=0; index_q < ppv->q_size_ddec[n_ddec]; index_q++){
          for(l=0; l<=ppv->l_max_ddec[n_ddec]; l++){
            if (l>2) ppv->used_in_sources[index_pt]=_FALSE_;
            index_pt++;
          }
        }
  ...
  if (pa_old == NULL) {
    ...
    if (_scalars_) {
      ...
      if (pba->has_ddec == _TRUE_) {
        ...
        class_test(ppw->approx[ppw->index_ap_ddecfa] == (int)ddecfa_on,
                   ppt->error_message,
                   "scalar initial conditions assume ddec fluid approximation turned off");
  ...
  /** - case of switching approximation while a wavenumber is being integrated */
    ...
    if (_scalars_) {
      ...
      if ((pa_old[ppw->index_ap_tca] == (int)tca_on) && (ppw->approx[ppw->index_ap_tca] == (int)tca_off)) {
        ...
        if (pba->has_ddec == _TRUE_) {
          ...
          for(n_ddec = 0; n_ddec < ppv->N_ddec; n_ddec++){
            for(index_q=0; index_q < ppv->q_size_ddec[n_ddec]; index_q++){
              for(l=0; l<=ppv->l_max_ddec[n_ddec];l++){
                // This is correct with or without ddecfa, since ppv->lmax_ddec is set accordingly.
                ppv->y[ppv->index_pt_psi0_ddec1+index_pt] =
                  ppw->pv->y[ppw->pv->index_pt_psi0_ddec1+index_pt];
                index_pt++;
              }
            }
          }
        ...
        if ((pa_old[ppw->index_ap_ufa] == (int)ufa_off) && (ppw->approx[ppw->index_ap_ufa] == (int)ufa_on)) {
          ...
          if (pba->has_ddec == _TRUE_) {
            ...
            for(n_ddec = 0; n_ddec < ppv->N_ddec; n_ddec++){
              for(index_q=0; index_q < ppv->q_size_ddec[n_ddec]; index_q++){
                for(l=0; l<=ppv->l_max_ddec[n_ddec]; l++){
                  /* This is correct even when ddecfa == off, since ppv->l_max_ddec and
                      ppv->q_size_ddec is updated.*/
                  ppv->y[ppv->index_pt_psi0_ddec1+index_pt] =
                    ppw->pv->y[ppw->pv->index_pt_psi0_ddec1+index_pt];
                  index_pt++;
                }
      ...
      /* -- case of switching on ddec fluid
         approximation. Provide correct initial conditions to new set
         of variables */

      if (pba->has_ddec == _TRUE_) {

        if ((pa_old[ppw->index_ap_ddecfa] == (int)ddecfa_off) && (ppw->approx[ppw->index_ap_ddecfa] == (int)ddecfa_on)) {

          if (ppt->perturbations_verbose>2)
            fprintf(stdout,"Mode k=%e: switch on ddec fluid approximation at tau=%e\n",k,tau);
          ...
          a = ppw->pvecback[pba->index_bg_a];
          index_pt = ppw->pv->index_pt_psi0_ddec1;
          for(n_ddec = 0; n_ddec < ppv->N_ddec; n_ddec++){
            // We are in the fluid approximation, so ddec_l_size is always 3.
            ddec_l_size = ppv->l_max_ddec[n_ddec]+1;
            rho_plus_p_ddec = ppw->pvecback[pba->index_bg_rho_ddec1+n_ddec]+
              ppw->pvecback[pba->index_bg_p_ddec1+n_ddec];
            for(l=0; l<=2; l++){
              ppv->y[ppv->index_pt_psi0_ddec1+ddec_l_size*n_ddec+l] = 0.0;
            }
            factor = pba->factor_ddec[n_ddec]*pow(pba->a_today/a,4);
            for(index_q=0; index_q < ppw->pv->q_size_ddec[n_ddec]; index_q++){
              // Integrate over distributions:
              q = pba->q_ddec[n_ddec][index_q];
              q2 = q*q;
              epsilon = sqrt(q2+a*a*pba->M_ddec[n_ddec]*pba->M_ddec[n_ddec]);
              ppv->y[ppv->index_pt_psi0_ddec1+ddec_l_size*n_ddec] +=
                pba->w_ddec[n_ddec][index_q]*q2*epsilon*
                ppw->pv->y[index_pt];

              ppv->y[ppv->index_pt_psi0_ddec1+ddec_l_size*n_ddec+1] +=
                pba->w_ddec[n_ddec][index_q]*q2*q*
                ppw->pv->y[index_pt+1];

              ppv->y[ppv->index_pt_psi0_ddec1+ddec_l_size*n_ddec+2] +=
                pba->w_ddec[n_ddec][index_q]*q2*q2/epsilon*
                ppw->pv->y[index_pt+2];

              //Jump to next momentum bin in ppw->pv->y:
              index_pt += (ppw->pv->l_max_ddec[n_ddec]+1);
            }
            ppv->y[ppv->index_pt_psi0_ddec1+ddec_l_size*n_ddec] *=factor/ppw->pvecback[pba->index_bg_rho_ddec1+n_ddec];
            ppv->y[ppv->index_pt_psi0_ddec1+ddec_l_size*n_ddec+1] *=k*factor/rho_plus_p_ddec;
            ppv->y[ppv->index_pt_psi0_ddec1+ddec_l_size*n_ddec+2] *=2.0/3.0*factor/rho_plus_p_ddec;
          }
        }
      }
    }
...
int perturb_vector_free(
  ...
  if (pv->l_max_ddec != NULL) free(pv->l_max_ddec);
  if (pv->q_size_ddec != NULL) free(pv->q_size_ddec);
  free(pv->y);
  free(pv->dy);
...
int perturb_initial_conditions(struct precision * ppr,
  ...
  if (_scalars_) {
    ...
    if (pba->has_ddec == _TRUE_) {
      for(n_ddec=0; n_ddec<pba->N_ddec; n_ddec++){
        rho_r += ppw->pvecback[pba->index_bg_rho_ddec1 + n_ddec];
        rho_nu += ppw->pvecback[pba->index_bg_rho_ddec1 + n_ddec];
      }
    }
  ...
  /*MS 11/8/2022 modify to have first order IC from Ma and Bertschinger for fluid*/
    ...
    if ((ppt->has_nid == _TRUE_) && (index_ic == ppt->index_ic_nid)) {
      ...
      class_test((pba->has_ur == _FALSE_) && (pba->has_ncdm == _FALSE_) && (pba->has_ddec == _FALSE_),
                 ppt->error_message,
    ...
    if (pba->has_ddec == _TRUE_) {
      idx = ppw->pv->index_pt_psi0_ddec1;
      for (n_ddec=0; n_ddec < pba->N_ddec; n_ddec++){
        ...
        for (index_q=0; index_q < ppw->pv->q_size_ddec[n_ddec]; index_q++) {
          ...
          epsilon = sqrt(q*q+a*a*pba->M_ddec[n_ddec]*pba->M_ddec[n_ddec]);

          ppw->pv->y[idx] = -0.25 * delta_ur * pba->dlnf0_dlnq_ddec[n_ddec][index_q];

          ppw->pv->y[idx+1] =  -epsilon/3./q/k*theta_ur* pba->dlnf0_dlnq_ddec[n_ddec][index_q];

          ppw->pv->y[idx+2] = -0.5 * shear_ur * pba->dlnf0_dlnq_ddec[n_ddec][index_q];

          ppw->pv->y[idx+3] = -0.25 * l3_ur * pba->dlnf0_dlnq_ddec[n_ddec][index_q];

          // PD: For recoupling we don't need this modification to the initial condition
          // TB: But for decoupling using tanh we should, right?
          //ddc start
          //if (ppr->ddec_fluid_approximation == ddecfa_fltofs1 || ppr->ddec_fluid_approximation == ddecfa_fltofs2){
          if (ppr->ddec_fluid_approximation == ddecfa_fltofs1){
            double z = 1./a-1.;
            double W_fluid = 0.5*(tanh((z-pba->zdec_ddec[n_ddec])/pba->dzdec_ddec[n_ddec])+1.);
            ppw->pv->y[idx+2] = ppw->pv->y[idx+2]*W_fluid;
            ppw->pv->y[idx+3] = ppw->pv->y[idx+3]*W_fluid;
          }
          //ddc end

          //Jump to next momentum bin:
          idx += (ppw->pv->l_max_ddec[n_ddec]+1);
...
int perturb_approximations(
  ...
  if (_scalars_) {
    ...
    if (pba->has_ddec == _TRUE_) {
      ...
      if ((tau/tau_k > ppr->ddec_fluid_trigger_tau_over_tau_k) &&
          (ppr->ddec_fluid_approximation != ddecfa_none)) {

        ppw->approx[ppw->index_ap_ddecfa] = (int)ddecfa_on;
      }
      else {
        ppw->approx[ppw->index_ap_ddecfa] = (int)ddecfa_off;
      }
...
int perturb_total_stress_energy(
  ...
  double rho_delta_ddec=0.;
  double rho_plus_p_theta_ddec=0.;
  double rho_plus_p_shear_ddec=0.;
  double delta_p_ddec=0.;
  double factor;
  ...
  double cg2_ddec,w_ddec,rho_ddec_bg,p_ddec_bg,pseudo_p_ddec;
  ...
  if (_scalars_) {
    ...
    /* non-cold dark matter (ddec) contribution */
    if (pba->has_ddec == _TRUE_) {
      idx = ppw->pv->index_pt_psi0_ddec1;
      if(ppw->approx[ppw->index_ap_ddecfa] == (int)ddecfa_on){
        // The perturbations are evolved integrated:
        for(n_ddec=0; n_ddec < pba->N_ddec; n_ddec++){
          rho_ddec_bg = ppw->pvecback[pba->index_bg_rho_ddec1+n_ddec];
          p_ddec_bg = ppw->pvecback[pba->index_bg_p_ddec1+n_ddec];
          pseudo_p_ddec = ppw->pvecback[pba->index_bg_pseudo_p_ddec1+n_ddec];

          rho_plus_p_ddec = rho_ddec_bg + p_ddec_bg;
          w_ddec = p_ddec_bg/rho_ddec_bg;
          cg2_ddec = w_ddec*(1.0-1.0/(3.0+3.0*w_ddec)*(3.0*w_ddec-2.0+pseudo_p_ddec/p_ddec_bg));
          if ((ppt->has_source_delta_ddec == _TRUE_) || (ppt->has_source_theta_ddec == _TRUE_) || (ppt->has_source_delta_m == _TRUE_)) {
            ppw->delta_ddec[n_ddec] = y[idx];
            ppw->theta_ddec[n_ddec] = y[idx+1];
            ppw->shear_ddec[n_ddec] = y[idx+2];
          }

          ppw->delta_rho += rho_ddec_bg*y[idx];
          ppw->rho_plus_p_theta += rho_plus_p_ddec*y[idx+1];
          ppw->rho_plus_p_shear += rho_plus_p_ddec*y[idx+2];
          ppw->delta_p += cg2_ddec*rho_ddec_bg*y[idx];
          rho_plus_p_tot += rho_plus_p_ddec;
        ...
        for(n_ddec=0; n_ddec < pba->N_ddec; n_ddec++){
          rho_delta_ddec = 0.0;
          rho_plus_p_theta_ddec = 0.0;
          rho_plus_p_shear_ddec = 0.0;
          delta_p_ddec = 0.0;
          factor = pba->factor_ddec[n_ddec]*pow(pba->a_today/a,4);
          ...
          for (index_q=0; index_q < ppw->pv->q_size_ddec[n_ddec]; index_q ++) {
            ...
            q = pba->q_ddec[n_ddec][index_q];
            q2 = q*q;
            epsilon = sqrt(q2+pba->M_ddec[n_ddec]*pba->M_ddec[n_ddec]*a2);

            rho_delta_ddec += q2*epsilon*pba->w_ddec[n_ddec][index_q]*y[idx];
            rho_plus_p_theta_ddec += q2*q*pba->w_ddec[n_ddec][index_q]*y[idx+1];
            rho_plus_p_shear_ddec += q2*q2/epsilon*pba->w_ddec[n_ddec][index_q]*y[idx+2];
            delta_p_ddec += q2*q2/epsilon*pba->w_ddec[n_ddec][index_q]*y[idx];

            //Jump to next momentum bin:
            idx+=(ppw->pv->l_max_ddec[n_ddec]+1);
          }

          rho_delta_ddec *= factor;
          rho_plus_p_theta_ddec *= k*factor;
          rho_plus_p_shear_ddec *= 2.0/3.0*factor;
          delta_p_ddec *= factor/3.;

          if ((ppt->has_source_delta_ddec == _TRUE_) || (ppt->has_source_theta_ddec == _TRUE_) || (ppt->has_source_delta_m == _TRUE_)) {
            ppw->delta_ddec[n_ddec] = rho_delta_ddec/ppw->pvecback[pba->index_bg_rho_ddec1+n_ddec];
            ppw->theta_ddec[n_ddec] = rho_plus_p_theta_ddec/
              (ppw->pvecback[pba->index_bg_rho_ddec1+n_ddec]+ppw->pvecback[pba->index_bg_p_ddec1+n_ddec]);
            ppw->shear_ddec[n_ddec] = rho_plus_p_shear_ddec/
              (ppw->pvecback[pba->index_bg_rho_ddec1+n_ddec]+ppw->pvecback[pba->index_bg_p_ddec1+n_ddec]);
          }

          ppw->delta_rho += rho_delta_ddec;
          ppw->rho_plus_p_theta += rho_plus_p_theta_ddec;
          ppw->rho_plus_p_shear += rho_plus_p_shear_ddec;
          ppw->delta_p += delta_p_ddec;
          rho_plus_p_tot += ppw->pvecback[pba->index_bg_rho_ddec1+n_ddec]+ppw->pvecback[pba->index_bg_p_ddec1+n_ddec];
        }
      }
    }
    ...
    */
    ...
    if (ppt->has_source_delta_m == _TRUE_) {
      ...
      /* include any other species non-relativistic today (like ddec species) */

      if (pba->has_ddec == _TRUE_) {

        for(n_ddec=0; n_ddec < pba->N_ddec; n_ddec++){

          delta_rho_m += ppw->pvecback[pba->index_bg_rho_ddec1+n_ddec]*ppw->delta_ddec[n_ddec];
          rho_m += ppw->pvecback[pba->index_bg_rho_ddec1+n_ddec];
        }
      }
    ...
    if ((ppt->has_source_delta_m == _TRUE_) || (ppt->has_source_theta_m == _TRUE_)) {
      ...
      if (pba->has_ddec == _TRUE_) {
        for(n_ddec=0; n_ddec < pba->N_ddec; n_ddec++){
          rho_plus_p_theta_m += (ppw->pvecback[pba->index_bg_rho_ddec1+n_ddec]+ppw->pvecback[pba->index_bg_p_ddec1+n_ddec])*ppw->theta_ddec[n_ddec];
          rho_plus_p_m += (ppw->pvecback[pba->index_bg_rho_ddec1+n_ddec]+ppw->pvecback[pba->index_bg_p_ddec1+n_ddec]);
        }
      }
  ...
  if (_tensors_) {
    ...
    if (ppt->evolve_tensor_ur == _TRUE_){
      ...
      if (ppt->tensor_method == tm_massless_approximation) {
        ...
        if (pba->has_ddec == _TRUE_) {
          for(n_ddec = 0; n_ddec < pba->N_ddec; n_ddec++) {
            /* (3 p_ddec1) is the "relativistic" contribution to rho_ddec1 */
            rho_relativistic += 3.*ppw->pvecback[pba->index_bg_p_ddec1+n_ddec];
          }
        }
      }
    ...
    /** - --> ddec contribution to gravitational wave source: */
    if (ppt->evolve_tensor_ddec == _TRUE_){
      ...
      for(n_ddec=0; n_ddec < pba->N_ddec; n_ddec++){
        ...
        for (index_q=0; index_q < ppw->pv->q_size_ddec[n_ddec]; index_q ++) {
          ...
          gwddec += q2*q2/epsilon*pba->w_ddec[n_ddec][index_q]*(1./15.*y[idx]+2./21.*y[idx+2]+1./35.*y[idx+4]);

          //Jump to next momentum bin:
          idx+=(ppw->pv->l_max_ddec[n_ddec]+1);
        }

        gwddec *= -_SQRT6_*4*a2*factor;

        ppw->gw_source += gwddec;
...
int perturb_sources(
  ...
  if (_scalars_) {
    ...
    /* delta_ddec1 */
    if (ppt->has_source_delta_ddec == _TRUE_) {
      for (index_type = ppt->index_tp_delta_ddec1; index_type < ppt->index_tp_delta_ddec1+pba->N_ddec; index_type++) {
        _set_source_(index_type) = ppw->delta_ddec[index_type - ppt->index_tp_delta_ddec1];
      }
    }
    ...
    /* theta_ddec1 */
    if (ppt->has_source_theta_ddec == _TRUE_) {
      for (index_type = ppt->index_tp_theta_ddec1; index_type < ppt->index_tp_theta_ddec1+pba->N_ddec; index_type++) {
        _set_source_(index_type) = ppw->theta_ddec[index_type - ppt->index_tp_theta_ddec1];
      }
    }
  }
...
int perturb_print_variables(double tau,
  ...
  /** - ddec sector begins */
  int n_ddec;
  double *delta_ddec=NULL, *theta_ddec=NULL, *shear_ddec=NULL, *delta_p_over_delta_rho_ddec=NULL;
  double rho_ddec_bg, p_ddec_bg, pseudo_p_ddec, w_ddec;
  double rho_delta_ddec = 0.0;
  double rho_plus_p_theta_ddec = 0.0;
  double rho_plus_p_shear_ddec = 0.0;
  double delta_p_ddec = 0.0;
  /** - ddec sector ends */
  ...
  if (pba->has_ddec == _TRUE_){
    class_alloc(delta_ddec, sizeof(double)*pba->N_ddec,error_message);
    class_alloc(theta_ddec, sizeof(double)*pba->N_ddec,error_message);
    class_alloc(shear_ddec, sizeof(double)*pba->N_ddec,error_message);
    class_alloc(delta_p_over_delta_rho_ddec, sizeof(double)*pba->N_ddec,error_message);
  }
  ...
  if (_scalars_) {
    ...
    if (pba->has_ddec == _TRUE_) {
      ...
      if(ppw->approx[ppw->index_ap_ddecfa] == (int)ddecfa_on){
        // The perturbations are evolved integrated:
        for(n_ddec=0; n_ddec < pba->N_ddec; n_ddec++){
          rho_ddec_bg = pvecback[pba->index_bg_rho_ddec1+n_ddec];
          p_ddec_bg = pvecback[pba->index_bg_p_ddec1+n_ddec];
          pseudo_p_ddec = pvecback[pba->index_bg_pseudo_p_ddec1+n_ddec];
          w_ddec = p_ddec_bg/rho_ddec_bg;

          delta_ddec[n_ddec] = y[idx];
          theta_ddec[n_ddec] = y[idx+1];
          shear_ddec[n_ddec] = y[idx+2];
          //This is the adiabatic sound speed:
          delta_p_over_delta_rho_ddec[n_ddec] = w_ddec*(1.0-1.0/(3.0+3.0*w_ddec)*(3.0*w_ddec-2.0+pseudo_p_ddec/p_ddec_bg));
          idx += ppw->pv->l_max_ddec[n_ddec]+1;
        }
      }
      else{
        ...
        for(n_ddec=0; n_ddec < pba->N_ddec; n_ddec++){
          ...
          delta_ddec[n_ddec] = rho_delta_ddec/ppw->pvecback[pba->index_bg_rho_ddec1+n_ddec];
          theta_ddec[n_ddec] = rho_plus_p_theta_ddec/
            (ppw->pvecback[pba->index_bg_rho_ddec1+n_ddec]+ppw->pvecback[pba->index_bg_p_ddec1+n_ddec]);
          shear_ddec[n_ddec] = rho_plus_p_shear_ddec/
            (ppw->pvecback[pba->index_bg_rho_ddec1+n_ddec]+ppw->pvecback[pba->index_bg_p_ddec1+n_ddec]);
          delta_p_over_delta_rho_ddec[n_ddec] = delta_p_ddec/rho_delta_ddec;
...
//     CLASS did not actually do a gauge transform here
//      if (pba->has_ncdm == _TRUE_) {
//        for(n_ncdm=0; n_ncdm < pba->N_ncdm; n_ncdm++){
//	  
//	  /* MTL added below 6/14/2022 only works for relativistic!!!!  */
//	  
//	  delta_ncdm[n_ncdm] -= 4. * pvecback[pba->index_bg_H]*pvecback[pba->index_bg_a]*alpha;
//          /** - --> Do gauge transformation of delta, deltaP/rho (?) and theta using -= 3aH(1+w_ncdm) alpha for delta. */
//        }
//      }
//      if (pba->has_ddec == _TRUE_) {
//        for(n_ddec=0; n_ddec < pba->N_ddec; n_ddec++){
//	  delta_ddec[n_ddec] -= 4.* pvecback[pba->index_bg_H]*pvecback[pba->index_bg_a]*alpha;
//
//          /** - --> Do gauge transformation of delta, deltaP/rho (?) and theta using -= 3aH(1+w_ddec) alpha for delta. */
//        }
//      }
      
/// Below is bug fix to do correct transformation from Manuel Buen-Abad, implemented 6/16/2022
      if (pba->has_ncdm == _TRUE_) {
        for(n_ncdm=0; n_ncdm < pba->N_ncdm; n_ncdm++){
          delta_ncdm[n_ncdm] -= 3.*(1. + ppw->pvecback[pba->index_bg_p_ncdm1+n_ncdm]/ppw->pvecback[pba->index_bg_rho_ncdm1+n_ncdm])*pvecback[pba->index_bg_H]*pvecback[pba->index_bg_a]*alpha;
      ...
      if (pba->has_ddec == _TRUE_) {
        for(n_ddec=0; n_ddec < pba->N_ddec; n_ddec++){
          delta_ddec[n_ddec] -= 3.*(1. + ppw->pvecback[pba->index_bg_p_ddec1+n_ddec]/ppw->pvecback[pba->index_bg_rho_ddec1+n_ddec])*pvecback[pba->index_bg_H]*pvecback[pba->index_bg_a]*alpha;
          theta_ddec[n_ddec] += k*k*alpha;
        }
      }      
    ...
    if ((pba->has_ddec == _TRUE_) && ((ppt->has_density_transfers == _TRUE_) || (ppt->has_velocity_transfers == _TRUE_) || (ppt->has_source_delta_m == _TRUE_))) {
      for(n_ddec=0; n_ddec < pba->N_ddec; n_ddec++){
        class_store_double(dataptr, delta_ddec[n_ddec], _TRUE_, storeidx);
        class_store_double(dataptr, theta_ddec[n_ddec], _TRUE_, storeidx);
        class_store_double(dataptr, shear_ddec[n_ddec], _TRUE_, storeidx);
        class_store_double(dataptr, delta_p_over_delta_rho_ddec[n_ddec],  _TRUE_, storeidx);
      }
    }
    /* Decaying cold dark matter */
  ...
  if (pba->has_ddec == _TRUE_){
    ...
    free(theta_ddec);
    free(shear_ddec);
    free(delta_p_over_delta_rho_ddec);
  }
...
int perturb_derivs(double tau,
  ...
  /* for use with non-cold dark matter (ddec): */
  int n_ddec;
  double rho_ddec_bg,p_ddec_bg,pseudo_p_ddec,w_ddec,ca2_ddec,ceff2_ddec=0.,cvis2_ddec=0.;
  ...
  if (_scalars_) {
    ...
    /** - ---> non-cold dark matter (ddec): massive neutrinos, WDM, etc. */
    //TBC: curvature in all ddec
    if (pba->has_ddec == _TRUE_) {

      idx = pv->index_pt_psi0_ddec1;

      /** - ----> first case: use a fluid approximation (ddecfa) */
      //TBC: curvature
      if(ppw->approx[ppw->index_ap_ddecfa] == (int)ddecfa_on) {

        /** - -----> loop over species */

        for (n_ddec=0; n_ddec<pv->N_ddec; n_ddec++) {

          /** - -----> define intermediate quantitites */

          rho_ddec_bg = pvecback[pba->index_bg_rho_ddec1+n_ddec]; /* background density */
          p_ddec_bg = pvecback[pba->index_bg_p_ddec1+n_ddec]; /* background pressure */
          pseudo_p_ddec = pvecback[pba->index_bg_pseudo_p_ddec1+n_ddec]; /* pseudo-pressure (see CLASS IV paper) */
          w_ddec = p_ddec_bg/rho_ddec_bg; /* equation of state parameter */
          ca2_ddec = w_ddec/3.0/(1.0+w_ddec)*(5.0-pseudo_p_ddec/p_ddec_bg); /* adiabatic sound speed */
          ...
          /* different ansatz for sound speed c_eff and viscosity speed c_vis */
          if (ppr->ddec_fluid_approximation == ddecfa_mb) {
            ceff2_ddec = ca2_ddec;
            cvis2_ddec = 3.*w_ddec*ca2_ddec;
          }
          if (ppr->ddec_fluid_approximation == ddecfa_hu) {
            ceff2_ddec = ca2_ddec;
            cvis2_ddec = w_ddec;
          }
          if (ppr->ddec_fluid_approximation == ddecfa_CLASS) {
            ceff2_ddec = ca2_ddec;
            cvis2_ddec = 3.*w_ddec*ca2_ddec;
          }
          // ddc start
          // Usually we set the values for when this should kick in so high that it never does, which is
          // necessary because I could not get the code to provide consistent results with the fluid approximation
          //if (ppr->ddec_fluid_approximation == ddecfa_fltofs1) {
          if (ppr->ddec_fluid_approximation == ddecfa_fltofs1 || ppr->ddec_fluid_approximation == ddecfa_fltofs2 || ppr->ddec_fluid_approximation == ddecfa_fltofs3) {
            ceff2_ddec = ca2_ddec;
            double z = 1./a-1.;
            double W_fluid = 0.5*(tanh((z-pba->zdec_ddec[n_ddec])/pba->dzdec_ddec[n_ddec])+1.);
            cvis2_ddec = 3.*w_ddec*ca2_ddec*W_fluid;
          }
          // ddc end

          /** - -----> exact continuity equation */

          dy[idx] = -(1.0+w_ddec)*(y[idx+1]+metric_continuity)-
            3.0*a_prime_over_a*(ceff2_ddec-w_ddec)*y[idx];

          /** - -----> exact euler equation */

          dy[idx+1] = -a_prime_over_a*(1.0-3.0*ca2_ddec)*y[idx+1]+
            ceff2_ddec/(1.0+w_ddec)*k2*y[idx]-k2*y[idx+2]
            + metric_euler;
          ...
          if (ppr->ddec_fluid_approximation == ddecfa_mb) {
            ...
            dy[idx+2] = -3.0*(a_prime_over_a*(2./3.-ca2_ddec-pseudo_p_ddec/p_ddec_bg/3.)+1./tau)*y[idx+2]
              +8.0/3.0*cvis2_ddec/(1.0+w_ddec)*s_l[2]*(y[idx+1]+metric_shear);

          }

          if (ppr->ddec_fluid_approximation == ddecfa_hu) {

            dy[idx+2] = -3.0*a_prime_over_a*ca2_ddec/w_ddec*y[idx+2]
              +8.0/3.0*cvis2_ddec/(1.0+w_ddec)*s_l[2]*(y[idx+1]+metric_shear);
          ...
          if (ppr->ddec_fluid_approximation == ddecfa_CLASS) {
            ...
            dy[idx+2] = -3.0*(a_prime_over_a*(2./3.-ca2_ddec-pseudo_p_ddec/p_ddec_bg/3.)+1./tau)*y[idx+2]
              +8.0/3.0*cvis2_ddec/(1.0+w_ddec)*s_l[2]*(y[idx+1]+metric_ufa_class);

          }

          // ddc start
          // TB: I'm not sure why this was set to only trigger for the first one, but since we don't
          // let the fluid approximation kick in it shouldn't matter
          if (ppr->ddec_fluid_approximation == ddecfa_fltofs1) {
      ...
      /** - ----> second case: use exact equation (Boltzmann hierarchy on momentum grid) */
        ...
        for (n_ddec=0; n_ddec<pv->N_ddec; n_ddec++) {
          ...
          for (index_q=0; index_q < pv->q_size_ddec[n_ddec]; index_q++) {

            /** - -----> define intermediate quantities */

            dlnf0_dlnq = pba->dlnf0_dlnq_ddec[n_ddec][index_q];
            q = pba->q_ddec[n_ddec][index_q];
            epsilon = sqrt(q*q+a2*pba->M_ddec[n_ddec]*pba->M_ddec[n_ddec]);
            qk_div_epsilon = k*q/epsilon;

            /** - -----> ddec density for given momentum bin */

            dy[idx] = -qk_div_epsilon*y[idx+1]+metric_continuity*dlnf0_dlnq/3.;

            /** - -----> ddec velocity for given momentum bin */

            dy[idx+1] = qk_div_epsilon/3.0*(y[idx] - 2*s_l[2]*y[idx+2])
              -epsilon*metric_euler/(3*q*k)*dlnf0_dlnq;

            /** - -----> ddec shear for given momentum bin */

            dy[idx+2] = qk_div_epsilon/5.0*(2*s_l[2]*y[idx+1]-3.*s_l[3]*y[idx+3])
              -s_l[2]*metric_shear*2./15.*dlnf0_dlnq;

            /** - -----> ddec l>3 for given momentum bin */

            for(l=3; l<pv->l_max_ddec[n_ddec]; l++){
              dy[idx+l] = qk_div_epsilon/(2.*l+1.0)*(l*s_l[l]*y[idx+(l-1)]-(l+1.)*s_l[l+1]*y[idx+(l+1)]);
            }

            dy[idx+l] = qk_div_epsilon*y[idx+l-1]-(1.+l)*k*cotKgen*y[idx+l];
            /** - -----> ddec lmax for given momentum bin (truncation as in Ma and Bertschinger)
                but with curvature taken into account a la arXiv:1305.3261 */

            // ddc start
            /** assume higher moments are controled by tanh function...*/
            // TB: The first option is the usual tanh one from Choi et al. 1804.10180 and Brinckmann et al. 2012.11830
            if (ppr->ddec_fluid_approximation == ddecfa_fltofs1){
              double z = 1./a-1.;
              double W_fluid = 0.5*(tanh((pba->zdec_ddec[n_ddec]-z)/pba->dzdec_ddec[n_ddec])+1.);
              for(l=2; l<pv->l_max_ddec[n_ddec]; l++){
                dy[idx+l] = dy[idx+l]*W_fluid;
              }
            }
            //
            // PD: Exact Boltzmann hierarchy starts here. Here we follow Oldengott et al. 1706.02123, which has slightly different definination of alpha_l than 1704.06657
            // Since tau_dot scales like a^-4, it can be written as (zdec/a)^4 and here I simply used the old input constant "zdec" to account for the interaction rate. In this definition, tau_dot=G_eff^2 T_{nu0}^5/a^4=(zdec/a)^4. So zdec=3.85137e-5 for G_eff=(10MeV)^-2 and tau_dot is in unit of Mpc^-1.
            // "dzdec"is now used as the cap for tau_dot, which can be modified in the .ini file. I set it to 1e10.
            if (ppr->ddec_fluid_approximation == ddecfa_fltofs2){
              //double tau_dot = pow(pba->zdec_ddec[n_ddec]/a,4);
              // The constant 7.4305e-16 comes from (eV/MeV)^4 (kB in eV/K)^5 (Mpc*eV).
              // Now the input parameter zdec is in unit of MeV^-2.
              double tau_dot = pow(pba->zdec_ddec[n_ddec],2)*pow(pba->T_ddec[n_ddec]*pba->T_cmb,5)*7.4305e-16/pow(a,4);
              if (tau_dot > pba->dzdec_ddec[n_ddec]){
                tau_dot = pba->dzdec_ddec[n_ddec];
              }

              dy[idx+2] += -0.40*tau_dot*y[idx+2];
              dy[idx+3] += -0.43*tau_dot*y[idx+3];
              dy[idx+4] += -0.46*tau_dot*y[idx+4];
              dy[idx+5] += -0.47*tau_dot*y[idx+5];
              for (l=6; l<= pv->l_max_ddec[n_ddec]; l++){
                dy[idx+l] += -0.48*tau_dot*y[idx+l];
              }
            }
            // TB: This uses the approximation of Lancaster et al. 1704.06657 (which I think is the same as Cyr-Racine & Sigurdson 1306.1536?)
            // Note that values for "zdec" now have a different normalization compared to ddecfa_fltofs2.
            if (ppr->ddec_fluid_approximation == ddecfa_fltofs3){
              //double tau_dot = pow(pba->zdec_ddec[n_ddec]/a,4);
              // The constant 7.4305e-16 comes from (eV/MeV)^4 (kB in eV/K)^5 (Mpc*eV).
              // Now the input parameter zdec is in unit of MeV^-2.
              double tau_dot = pow(pba->zdec_ddec[n_ddec],2)*pow(pba->T_ddec[n_ddec]*pba->T_cmb,5)*7.4305e-16/pow(a,4);
              ...
              for (l=2; l<= pv->l_max_ddec[n_ddec]; l++){
                dy[idx+l] += -1.*tau_dot*y[idx+l];
              }

            }

           if (ppr->ddec_fluid_approximation == recfa_fltofs1){
              //Simplified recoupling Boltzmann hierarchy with all alpha_ell=1
              //
              double tau_dot = pba->zdec_ddec[n_ddec];
           ...
           if (ppr->ddec_fluid_approximation == recfa_fltofs2){
              //Simplified recoupling Boltzmann hierarchy with all alpha_2=1, alpha_3=1.5 and alpha_l>3=2

              double tau_dot = pba->zdec_ddec[n_ddec];


               dy[idx+2] += -1.*tau_dot*y[idx+2];
              dy[idx+3] += -1.5*tau_dot*y[idx+3];
              dy[idx+4] += -2.*tau_dot*y[idx+4];
              for (l=5; l<= pv->l_max_ddec[n_ddec]; l++){
                dy[idx+l] += -2.*tau_dot*y[idx+l];
              }

            }
           // ddc end

           //moved this up:  dy[idx+l] = qk_div_epsilon*y[idx+l-1]-(1.+l)*k*cotKgen*y[idx+l];
    ...
    if (ppt->evolve_tensor_ddec == _TRUE_) {
      ...
      for (n_ddec=0; n_ddec<pv->N_ddec; n_ddec++) {
        ...
        for (index_q=0; index_q < pv->q_size_ddec[n_ddec]; index_q++) {
          ...
          /** - ----> ddec l>0 for given momentum bin */

          for(l=1; l<pv->l_max_ddec[n_ddec]; l++){
            dy[idx+l] = qk_div_epsilon/(2.*l+1.0)*(l*s_l[l]*y[idx+(l-1)]-(l+1.)*s_l[l+1]*y[idx+(l+1)]);
          }
