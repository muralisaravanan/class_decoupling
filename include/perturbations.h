...
enum ddecfa_flags {ddecfa_off, ddecfa_on};
...
enum ddecfa_method {ddecfa_mb,ddecfa_hu,ddecfa_CLASS,ddecfa_fltofs1,ddecfa_fltofs2,ddecfa_fltofs3,recfa_fltofs1,recfa_fltofs2,ddecfa_none};
...
struct perturbs
{
  ...
  short evolve_tensor_ddec;             /**< (ddec) will we evolve ncdm tensor perturbations (if we have ncdm species and we use the exact method) ? */
 ...
 int size_tensor_perturbation_data[_MAX_NUMBER_OF_K_FILES_]; /**< Array of sizes of tensor double pointers  */
  ...
  short has_source_delta_ddec; /**< (ddec) do we need source for delta of all non-cold dark matter species (e.g. massive neutrinos)? */
  ...
  short has_source_theta_ddec; /**< (ddec) do we need source for theta of all non-cold dark matter species (e.g. massive neutrinos)? */
  ...
  int index_tp_delta_ddec1; /**< (ddec) index value for delta of first non-cold dark matter species (e.g. massive neutrinos) */
  ...
  int index_tp_theta_ddec1;/**< (ddec) index value for theta of first non-cold dark matter species (e.g. massive neutrinos) */
...
/* perturbed recombination */
  ...
  int index_pt_psi0_ddec1; /**< (ddec) first multipole of perturbation of first ncdm species, Psi_0 */
  int N_ddec;		/**< (ddec) number of distinct non-cold-dark-matter (ncdm) species */
  int* l_max_ddec;	/**< (ddec) mutipole l at which Boltzmann hierarchy is truncated (for each ncdm species) */
  int* q_size_ddec;	/**< (ddec) number of discrete momenta (for each ncdm species) */
...
struct perturb_workspace
{
  ...
  */
  ...
  double * delta_ddec;	/**< (ddec) relative density perturbation of each ncdm species */
  double * theta_ddec;	/**< (ddec) velocity divergence theta of each ncdm species */
  double * shear_ddec;	/**< (ddec) shear for each ncdm species */
  ...
  int index_ap_ddecfa; /**< (ddec) index for ncdm fluid approximation */
